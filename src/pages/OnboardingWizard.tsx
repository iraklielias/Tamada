...
const REGIONS: { id: GeorgianRegion; name_ka: string; name_en: string; desc: string }[] = [
  { id: "kakheti", name_ka: "კახეთი", name_en: "Kakheti", desc: "Wine country — formal & poetic" },
  { id: "imereti", name_ka: "იმერეთი", name_en: "Imereti", desc: "Warm & witty style" },
  { id: "kartli", name_ka: "ქართლი", name_en: "Kartli", desc: "Classical tradition" },
  { id: "racha", name_ka: "რაჭა-ლეჩხუმი", name_en: "Racha-Lechkhumi", desc: "Heartfelt & humorous" },
  { id: "samegrelo", name_ka: "სამეგრელო", name_en: "Samegrelo", desc: "Expressive & bold" },
  { id: "guria", name_ka: "გურია", name_en: "Guria", desc: "Cheerful & energetic" },
  { id: "adjara", name_ka: "აჭარა", name_en: "Adjara", desc: "Rich coastal tradition" },
  { id: "svaneti", name_ka: "სვანეთი", name_en: "Svaneti", desc: "Ancient & ceremonial" },
  { id: "meskheti", name_ka: "მესხეთი", name_en: "Meskheti", desc: "Deeply traditional" },
];
...
const OnboardingWizard: React.FC = () => {
  ...
  const handleComplete = async () => {
    ...
    if (error) {
      toast.error(t("onboarding.errorOccurred", "შეცდომა მოხდა. სცადეთ თავიდან."));
      setSaving(false);
      return;
    }

    await refreshProfile();
    toast.success(t("onboarding.profileReady", "გილოცავთ! თქვენი პროფილი მზადაა 🍷"));
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-caption text-muted-foreground">
                  {t("onboarding.step", "ნაბიჯი")} {step}/{TOTAL_STEPS}
                </p>
              </div>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="text-center">
                  <h1 className="font-display text-heading-1 text-foreground mb-2">
                    {t("onboarding.welcome", "მოგესალმებით, თამადა!")} 🍷
                  </h1>
                  <p className="text-body text-muted-foreground">
                    {t("onboarding.welcomeDesc", "Let's set up your profile.")}
                  </p>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-card border border-border">
                  ...
                  >
                    {t("onboarding.yourName", "თქვენი სახელი")} *
                  </Label>
                  <Input
                    id="displayName"
                    value={displayName}
                    onChange={(e) => setDisplayName(e.target.value)}
                    placeholder={t("onboarding.namePlaceholder", "მაგ. გიორგი")}
                  ...
                  <Label className="text-body-sm text-foreground font-semibold mt-5 block">
                    {t("onboarding.language", "ენა / Language")}
                  </Label>
                  <div className="flex gap-3 mt-2">
                    {(["ka", "en"] as const).map((lang) => (
                      ...
                      >
                        {lang === "ka" ? t("onboarding.langKa", "🇬🇪 ქართული") : t("onboarding.langEn", "🇬🇧 English")}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                >
                  {t("onboarding.continue", "გაგრძელება")} <ChevronRight className="ml-1 h-4 w-4" />
                </Button>
              </motion.div>
            )}
              ...
              >
                <div className="text-center">
                  <h2 className="font-display text-heading-1 text-foreground mb-2">
                    {t("onboarding.yourRegion", "თქვენი რეგიონი")}
                  </h2>
                  <p className="text-body text-muted-foreground">
                    {t("onboarding.regionDesc", "აირჩიეთ თქვენთვის სასურველი ღვინის რეგიონი")}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {REGIONS.map((r) => (
                    ...
                    >
                      <p className="font-semibold text-foreground text-body-sm">
                        {i18n.language === "en" ? r.name_en : r.name_ka}
                      </p>
                  ...
                  >
                    <p className="font-semibold text-foreground text-body-sm">
                      {t("onboarding.noPreference", "არ მაქვს უპირატესობა")}
                    </p>
                  </button>
                </div>
            ...
            {step === 3 && (
              ...
              >
                <div className="text-center">
                  <h2 className="font-display text-heading-1 text-foreground mb-2">
                    {t("onboarding.experience", "გამოცდილება")}
                  </h2>
                  <p className="text-body text-muted-foreground">
                    {t("onboarding.experienceDesc", "როგორი გამოცდილება გაქვთ სუფრის მართვაში?")}
                  </p>
                </div>
                  ...
                  {EXPERIENCE_LEVELS.map((lvl) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-foreground">{t(`onboarding.experienceLevels.${lvl.id}`, lvl.name_ka)}</p>
                        <p className="text-caption text-muted-foreground">
                          {t(`onboarding.experienceLevels.${lvl.id}Desc`, lvl.desc_ka)}
                        </p>
                      </div>
                      {experienceLevel === lvl.id && (
                        <motion.div
            ...
            {step === 4 && (
              ...
              >
                <div className="text-center">
                  <h2 className="font-display text-heading-1 text-foreground mb-2">
                    {t("onboarding.occasions", "შემთხვევები")}
                  </h2>
                  <p className="text-body text-muted-foreground">
                    {t("onboarding.occasionsDesc", "რა ტიპის სუფრებს მართავთ ჩვეულებრივ? (მრავალი არჩევანი)")}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {OCCASIONS.map((occ) => (
                    ...
                    >
                      ...
                      <p className="font-semibold text-foreground text-body-sm">
                        {t(`onboarding.occasionTypes.${occ.id}`, occ.name_ka)}
                      </p>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {saving ? (
                      t("onboarding.saving", "შენახვა...")
                    ) : (
                      <>
                        <Sparkles className="h-4 w-4 mr-1" />
                        {t("onboarding.finish", "დასრულება")}
                      </>
                    )}
                  </Button>
                  <Button
